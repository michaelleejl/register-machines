type message = { at : Text.Span.t; text : string }
type t = { summary : string; label : message; notes : message list }

let handlers : (exn -> t option) list ref = ref []
let register handler = handlers := handler :: !handlers
let try_run e = List.find_map (fun handler -> handler e) !handlers
...
let line_at source (p : Lexing.position) =
  let stop =
    Option.value
      (String.index_from_opt source p.pos_bol '\n')
...
let block ~source ~severity ~summary ~label (span : Text.Span.t) =
  ...
  String.concat "\n"
    [
      ...
      Printf.sprintf "%s | %s" gutter
        (if label = "" then caret else caret ^ " " ^ label);
    ]

let render ~source { summary; label; notes } =
  let note { at; text } =
    block ~source ~severity:"note" ~summary:text ~label:"" at
  in
  String.concat "\n\n"
