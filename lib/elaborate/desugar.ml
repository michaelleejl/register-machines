...
let program { registers; instructions } =
  ...
  let registers =
    List.map
      (fun (r : register) -> Definitional.{ name = r.name; value = r.value })
      registers
  in
  let registers =
    if has_jump then registers @ [ Definitional.{ name = "_zero"; value = 0 } ]
    else registers
  in
  Definitional.{ registers; instructions = List.map translate instructions }
