...
let rename renamings registers (instruction : instruction) =
  let rename_register r =
    ...
    | None -> r
  in
  let body = match instruction.body with
    ...
    | ChExecute { machine; arguments; next } ->
      ...
      in
      LExecute { machine; arguments = List.map rename_register arguments; next }
  in
...
and machine prefix renamings lifted (m : machine) =
  ...
  let renamings', lifted = machines name renamings lifted m.definition.machines in
  let registers = List.map (qualify name) m.definition.registers in
  let register_renamings = StringMap.of_list (List.combine m.definition.registers registers) in
  let instructions = List.map (rename renamings' register_renamings) m.definition.instructions in
  let lifted_machine = Lifted.{ name; parameters = m.parameters; registers; instructions } in
...
let program ({ machines = ms; registers; instructions } : program) =
  ...
  let registers =
    List.map (fun (r : register) -> Lifted.{ name = r.name; value = r.value }) registers
  in
  Lifted.{ machines = List.rev lifted;
           registers;
